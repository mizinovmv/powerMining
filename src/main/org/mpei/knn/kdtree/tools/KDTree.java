...
public class KDTree {
	...
	public void setRoot(KDNode m_root) {
		this.m_root = m_root;
	}
	// count of nodes
	private int m_count;
	...
	public void setCountNodes(int m_count) {
	...
	public void insert(double[] key, Object value) {
		...
		if (key.length != m_K) {
			...
			m_root = KDNode.ins(new DenseVector(key), value, m_root, 0, m_K);
	...
	public Object search(double[] key) {
		...
		KDNode kd = KDNode.srch(new DenseVector(key), m_root, m_K);
	...
	public void delete(double[] key) {
		...
		if (key.length != m_K) {
			...
			KDNode t = KDNode.srch(new DenseVector(key), m_root, m_K);
			if (t == null) {
	...
	public Object[] nearest(double[] key, int n) {
		...
		if (n < 0 || n > m_count) {
			throw new IllegalArgumentException("Number of neighbors (" + n
					+ ") cannot"
...
//		HPoint keyp = new HPoint(key);
		org.apache.mahout.math.Vector keyp = new DenseVector(key);
	...
	public Object[] range(double[] lowk, double[] uppk) {
		...
		if (lowk.length != uppk.length) {
			...
			KDNode.rsearch(new DenseVector(lowk), new DenseVector(uppk), m_root, 0, m_K,
					v);
