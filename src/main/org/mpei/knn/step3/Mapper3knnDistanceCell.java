...
public class Mapper3knnDistanceCell extends
		Mapper<LongWritable, MapWritable, Text, IntWritable> {

	private static final Logger log = LoggerFactory.getLogger(Mapper3knnDistanceCell.class);
	...
	protected void map(LongWritable key, MapWritable value, Context context)
		...
		int knn = Integer.valueOf(context.getConfiguration().get(
				"knnmapper.numberNN"));
		// knnDistance
		TreeMap<Double, String> NN = new TreeMap<Double, String>();

		ArrayList<String> NNCell = new ArrayList<String>();

		double step = 0;
		double dStep = 0;
		while ((dStep = step / Math.pow(10, delimiter)) < 1) {
				...
				try {
					...
					for (String nnCell : NNCell) {
						...
						for (Object tmp : jsonDocs) {
							...
							// knnDistance
							docCoordinates = jsonDoc.get("coordinates")
									.toString();
							String[] coordinates = docCoordinates
									.split(separator);
							double distanceValue = distance(pointCoordinates,
									coordinates);
							if (NN.size() < knn) {
								NN.put(distanceValue, docClass);
							} else {
								if (distanceValue < NN.lastKey()) {
					...
					for (Map.Entry<Double,String> neighbor : NN.entrySet()) {
						String nbClass = neighbor.getValue();
...
//					}
//					Collections.sort(NNLablesSort,new Comparator<Map.Entry<String, Integer>>() {
...
//						
//					});
//					String nnName = NNLablesSort.get(0).getKey();
//					if (nnName.compareTo(className) == 0) {
...
//					}
					...
					for (Map.Entry<String, Integer> entry : NNLables.entrySet()) {
						if (maxEntry == null
	...
	private static double distance(ArrayList<Double> lhs, String[] rhs) {
		int i = 0;
		double sum = 0;
		for (String tmp : rhs) {
			double rhsValue = Double.valueOf(tmp);
			sum += Math.pow((rhsValue - lhs.get(i)), 2d);
			++i;
		}
