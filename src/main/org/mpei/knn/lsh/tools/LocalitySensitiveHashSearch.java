package org.mpei.knn.lsh.tools;
 ...
 * approximation to estimate distance without floating point math. The clever
 * bit about this implementation is that it does an adaptive cutoff for the
 * cutoff on the bitwise distance. Making this cutoff adaptive means that we
 * only needs to make a single pass through the data.
 */
public class LocalitySensitiveHashSearch extends UpdatableSearcher implements
	...
	// this matrix of 32 random vectors is used to compute the Locality
	// Sensitive Hash
	...
	public List<WeightedThing<Vector>> search(Vector q, int numberOfNeighbors) {
		...
		PriorityQueue<WeightedThing<Vector>> top = new PriorityQueue<WeightedThing<Vector>>(
				getSearchSize(), Ordering.natural().reverse());

		// we keep the counts of the hash distances here. This lets us
		// accurately
		...
		for (HashedVector v : trainingVectors) {
			...
			if (bitDot <= hashLimit) {
				...
				if (d < distanceLimit) {
					...
					limitCount++;
					while (hashLimit > 0
							&& limitCount - hashCounts[hashLimit - 1] > searchSize) {
						hashLimit--;
					...
					if (hashLimitStrategy >= 0) {
						while (hashLimit < 32
								&& distribution[hashLimit].getCount() > 10
								&& (hashLimitStrategy * distribution[hashLimit]
										.getQuartile(1))
										+ ((1 - hashLimitStrategy) * distribution[hashLimit]
												.getQuartile(0)) < distanceLimit) {
							limitCount += hashCounts[hashLimit];
							hashLimit++;
						}
					}
		...
		List<WeightedThing<Vector>> r = Lists.newArrayList(Iterables.transform(
				top,
				new Function<WeightedThing<Vector>, WeightedThing<Vector>>() {
					@Override
					public WeightedThing<Vector> apply(
							@Nullable WeightedThing<Vector> input) {
						return new WeightedThing<Vector>(((HashedVector) (input
								.getValue())).getVector(), input.getWeight());
					}
				}));
		Collections.sort(r);
	...
	public void add(Vector v) {
		initialize(v.size());
		trainingVectors.add(new HashedVector(v, projection,
				HashedVector.INVALID_INDEX, BITMASK));
	}
